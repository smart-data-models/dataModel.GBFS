...
dataModel = "system_pricing_plans"
...
data = {'type': 'Property', 'value': {'plans': [{'plan_id': 'plan3', 'name': 'Simple Rate', 'currency': 'CAD', 'price': 3, 'is_taxable': True, 'description': '$3 unlock fee, $0.25 per kilometer and 0.50 per minute.', 'per_km_pricing': [{'start': 0, 'rate': 0.25, 'interval': 1}], 'per_min_pricing': [{'start': 0, 'rate': 0.5, 'interval': 1}]}]}}
attribute = "data"
value = data
# The next line creates the query for inserting this attribute in a NGSI-LD context broker if the attribute does not exist it creates it
print(sdm.update_broker(dataModel, subject, attribute, value, serverUrl=serverUrl, updateThenCreate=True))
